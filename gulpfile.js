// generated on 2021-04-03 using generator-webapp 4.0.0-8
...
function scriptsPlugins() {
  return src('app/scripts/plugins/*.js', {
    sourcemaps: !isProd,
  })
    .pipe($.plumber())
    .pipe($.babel())
...
const build = series(
  ...
  parallel(
    lint,
    series(parallel(styles, scriptsPlugins, scripts, modernizr), html),
    images,
    fonts,
    extras
  ),
...
let serve;
if (isDev) {
  serve = series(clean, parallel(styles, scriptsPlugins, scripts, modernizr, fonts), startAppServer);
} else if (isTest) {
  serve = series(clean, scriptsPlugins, scripts, startTestServer);
} else if (isProd) {
